...
def train_one_epoch(model, loader, optimizer, criterion, device, lambda_boundary=1):
    model.train()
    running_loss = 0.0
    total_iterations = 0  
...
model = make_SegFormerB0_with_boundary(num_classes=6)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss or train_loss < best_train_loss:
        ...
        torch.save(model.state_dict(), f'./model_b0_struct/best_segmentation_model_{epoch}.pth')
